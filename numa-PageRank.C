...
void *PageRankSubWorker(void *arg) {
    ...
    while (1) {
        ...
        if (subworker.isSubMaster()) {
            ...
            //printf("time of %d: %lf\n", subworker.tid * N_USE_CORES_PER_NODE + subworker.subTid, duration);
        }
        ...
        vertexMap(Frontier, PR_Vertex_F(p_curr, p_next, damping, n), tid, subTid, N_USE_CORES_PER_NODE);
        //vertexCounter(GA, output, tid, subTid, N_USE_CORES_PER_NODE);
        output->m = 1;
...
//    N_USE_CORES_PER_NODE = numa_num_configured_cpus() / N_USE_NODES;
    ...
    intT subShardSize = ((GA.n / N_USE_NODES) / vertPerPage) * vertPerPage;
    for (int i = 0; i < N_USE_NODES - 1; i++) {
	sizeArr[i] = subShardSize;
    }
    sizeArr[N_USE_NODES - 1] = GA.n - subShardSize * (N_USE_NODES - 1);
    */
    int accum = 0;
